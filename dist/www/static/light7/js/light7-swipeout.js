...
+function ($) {
    ...
    $.initSwipeout = function (swipeoutEl) {
        ...
        $(document).on($.touchEvents.start, function (e) {
            if (swipeoutOpenedEl) {
                var target = $(e.target);
                if (!(swipeoutOpenedEl.is(target[0]) || target.parents('.swipeout').is(swipeoutOpenedEl) || target.hasClass('modal-in') || target.hasClass('modal-overlay') || target.hasClass('actions-modal') || target.parents('.actions-modal.modal-in, .modal.modal-in').length > 0)) {
...
//# sourceMappingURL=light7-swipeout.js.map